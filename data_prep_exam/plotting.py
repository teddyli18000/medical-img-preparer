...
def generate_figures(report_data: Dict[str, Any], config) -> Dict[str, Any]:
    ...
    image_dtype = stats.get("image_dtype_distribution", {})
    if image_dtype:
        items = sorted(image_dtype.items(), key=lambda item: (-item[1], item[0]))
        labels = [item[0] for item in items][: config.FIGURE_MAX_CATEGORIES]
        values = [int(item[1]) for item in items][: config.FIGURE_MAX_CATEGORIES]
        fig, ax = plt.subplots(figsize=fig_size)
        _bar(ax, labels, values, color, "Image Dtype Distribution", "dtype")
        _apply_axes_style(ax, grid)
        filename = "image_dtype_bar.png"
        path = os.path.join(figs_dir, filename)
        _save_fig(path, fig)
        _add_fig(
            os.path.relpath(path, config.REPORT_DIR),
            "Image Dtype Distribution",
            "Image dtype counts across all samples.",
        )

    label_dtype = stats.get("label_dtype_distribution", {})
    if label_dtype:
        items = sorted(label_dtype.items(), key=lambda item: (-item[1], item[0]))
        labels = [item[0] for item in items][: config.FIGURE_MAX_CATEGORIES]
        values = [int(item[1]) for item in items][: config.FIGURE_MAX_CATEGORIES]
        fig, ax = plt.subplots(figsize=fig_size)
        _bar(ax, labels, values, color, "Label Dtype Distribution", "dtype")
        _apply_axes_style(ax, grid)
        filename = "label_dtype_bar.png"
        path = os.path.join(figs_dir, filename)
        _save_fig(path, fig)
        _add_fig(
            os.path.relpath(path, config.REPORT_DIR),
            "Label Dtype Distribution",
            "Label dtype counts across all samples.",
        )
