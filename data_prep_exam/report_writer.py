...
def write_markdown_report(report_data: Dict[str, Any], output_path: str, config) -> None:
    ...
    lines.append(f"- Preprocessed Dir: {summary['preprocessed_dir']}")
    lines.append("")
    ...
    lines.append("## 6. Dtype Distribution")
    lines.append("")
    image_dtype_rows = [[k, v] for k, v in stats.get("image_dtype_distribution", {}).items()]
    label_dtype_rows = [[k, v] for k, v in stats.get("label_dtype_distribution", {}).items()]
    lines.append("### 6.1 Image Dtype")
    lines.append("")
    lines.extend(_render_table(["Dtype", "Count"], image_dtype_rows))
    lines.append("")
    lines.append("### 6.2 Label Dtype")
    lines.append("")
    lines.extend(_render_table(["Dtype", "Count"], label_dtype_rows))
    lines.append("")
    ...
    lines.append("## 8. Missing / Orphan Labels")
    lines.append("")
    ...
    lines.append("## 10. Sample-Level Details")
    lines.append("")
    sample_rows = []
    for item in samples:
        sample_rows.append(
            [
                ...
                item.get("image_dtype"),
                item.get("label_dtype"),
                item["read_status"],
                item["error_message"],
            ]
        )
    ...
    if dist_notes:
        lines.append("## 12. Distribution Notes")
        lines.append("")
